...
const icon = {
  plane: '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M3 20v-6l8-2l-8-2V4l19 8z"/></svg>',
...
export class NoteDetailOption extends HTMLElement {
  ...
  controlExternalInput(note) {
    ...
    textarea.classList.add(note.option.color);
  ...
  setupEvents() {
    ...
    this.shadowRoot.querySelectorAll(".color").forEach(el => {
      el.addEventListener("click", () => {
        $noteDetail.set({
          ...$noteDetail.get(),
          option: { ...$noteDetail.get().option, color: el.dataset.color }
        });
      });
    });
    ...
    this.shadowRoot.querySelector(".add-btn").addEventListener("click", () => {
      ...
      const newNote = { ...current, meta: { id: crypto.randomUUID(), createdAt: new Date().toISOString() } };
